...
public class InvertedIndexMovieSearchService implements MovieSearchService {
	...
	@Override
	public List<Movie> search(String queryText) throws ExecutionException, InterruptedException {
		...
		for (String word : queryText.split(" ")) {
			...
			if (invertedIndex != null) {
				List<Movie> movieList = movieRepository.findByIdIn(invertedIndex.getMovie_ids());
				result = intersecResult(result, movieList);
			}
